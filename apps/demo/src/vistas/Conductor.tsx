...
export function VistaConductor() {
  ...
  async function consultarSaldo() {
    setError(null);
    setSaldo(await client.consultarSesion(plate));
  }

  const monto = cotizacion?.amount ?? 0;

  return (
    <main className="mx-auto grid max-w-6xl gap-8 px-4 py-8 sm:px-6 lg:grid-cols-[.92fr_1.08fr] lg:items-center">
      <section className="min-w-0">
        <Badge tono="cyan">App Conductor · Municipalidad de Salta</Badge>
        ...
        <div className="relative overflow-hidden rounded-[1.7rem] bg-white p-4 text-[#0A1A2F] sm:p-5">
          ...
          {resultado ? (
            <div className="my-4">
              ...
              <Boton className="mt-4 w-full" variante="secundario" onClick={() => { setResultado(null); setSaldo(null); }}>Nuevo pago</Boton>
            </div>
          ) : (
            <>
              ...
              <Tarjeta className="border-slate-200 bg-slate-50 p-4 text-slate-900 shadow-none" titulo={<span className="text-slate-500">Pago de estacionamiento</span>}>
                ...
                <button onClick={consultarSaldo} className="mt-3 w-full text-center text-xs font-semibold text-[#0067B1] hover:underline">Consultar saldo de la patente</button>
              </Tarjeta>
            </>
          )}
