  ...
  return (
    <div className="space-y-8">
      <section className="grid gap-4 md:grid-cols-4">
        ...
        <Kpi label="Al Municipio" valor={formatARS(sp.municipio)} sub={`${SPLIT.municipioPct}% · recaudación municipal`} acento="texto" />
      </section>
          ...
          <Tarjeta>
            <div className="space-y-2">
              {datos.dashboard.porPermisionario.slice(0, 6).map((p, i) => (
                <div key={p.permisionarioId} className="flex items-center justify-between gap-3 rounded-xl bg-profundo/70 p-3 text-sm">
      ...
      <section className="grid gap-4 lg:grid-cols-[.9fr_1.1fr]">
        ...
        <Tarjeta titulo="Tesis comercial" accion={<Badge tono="cyan">Para el Municipio</Badge>}>
          <div className="grid gap-3 text-sm text-texto-tenue md:grid-cols-3">
            ...
            <p><b className="text-texto">Tránsito Municipal fiscaliza y recauda.</b> Puede controlar pagos, credenciales, patentes, sectores y espacios reservados, con trazabilidad por operación.</p>
            <p><b className="text-texto">La Municipalidad conserva ingresos.</b> El {SPLIT.municipioPct}% queda asignado al Municipio y cada pago genera comprobante digital auditable.</p>
          </div>
        </Tarjeta>
      </section>
    </div>
